...
const initialState = { name:'', email: '',post:'',mobileNumber:'', password: '', confirmPassword: '' };
  ...
  return (
    <form class="min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12" onSubmit={handleSubmit}>
      <div class="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
        <div className="mb-5 px-[125px] justify-center items-center">
          <Link to="/projects" className="flex justify-center gap-2">
            <img src={logo} alt="logo" className="w-9 h-9 object-contain rounded-full" />
            <p className="text-black text-[25px] font-semibold cursor-pointer hidden sm:block">Aeromodelling Club </p>
          </Link>
        </div>
        <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
          <div class="px-5 py-7">
            {isSignup && (
              <>
                ...
                <input name="name" onChange={handleChange} type="text" required class="bg-white text-black border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
                <label class="font-semibold text-sm text-gray-600 pb-1 block">Post</label>
                <input required name="post" onChange={handleChange} type="text" class="bg-white text-black border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
              </>
            )}
            <label class="font-semibold text-sm text-gray-600 pb-1 block">Mobile Number</label>
                <input name="mobileNumber" onChange={handleChange} type="text" required class="bg-white text-black border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
            ...
            <textarea id="comment" rows="6"
                ...
                class="px-0 w-full text-sm focus:ring-0 focus:outline-none bg-white text-black border rounded-lg  "
                placeholder="Description on executive..." required></textarea>
          ...
          <div class="py-5 px-5">
            <button onClick={handleSubmit} type="button" class="transition duration-200 bg-blue-500 hover:bg-blue-600 focus:bg-blue-700 focus:shadow-sm focus:ring-4 focus:ring-blue-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block">
              <span class="inline-block mr-2">{isSignup ? 'Create the Account' : 'Sign In'}</span>
        ...
        <div class="py-5">
          <div class="grid grid-cols-2 gap-1">
            <Link to="/projects" class="text-center sm:text-left whitespace-nowrap">
